...
output "kube_config" {
  ...
  value       = trimspace(<<-EOT
  %{if var.worker_desired_size == "0"~}${intersight_kubernetes_cluster_profile.cluster_without_worker[0].kube_config}
  %{else~}${intersight_kubernetes_cluster_profile.cluster_with_worker[0].kube_config}
  %{endif~}
  EOT
  )
}
